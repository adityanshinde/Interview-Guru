...
function createMemoryUser(userId, email) {
  const now = Date.now();
...
function toPublicSession(session, email = "") {
  return {
    ...
    duration_seconds: Math.max(0, Math.floor((Date.now() - session.startTime) / 1e3)),
    status: session.status
  };
}
async function getUserFromDB(userId) {
  const cachedUser = getMemoryUser(userId);
  if (cachedUser) {
    return cachedUser;
  }
  if (!isDBConnected()) {
...
async function createUserInDB(userId, email) {
  const user = createMemoryUser(userId, email);
  if (!isDBConnected()) {
    console.warn("[DB] \u26A0\uFE0F Database unavailable - using in-memory user store");
    return persistMemoryUser(user);
  }
  try {
    const query = `
      INSERT INTO users 
    ...
    const params = [
      ...
      user.sessionsUsed,
      new Date(user.trialStartDate || user.createdAt),
      user.trialsUsed,
    ...
    console.warn("[DB] \u26A0\uFE0F Falling back to in-memory user store");
    return persistMemoryUser(user);
  }
}
...
async function createSession(userId) {
  try {
    ...
    if (!isDBConnected()) {
      persistMemorySession(sessionRecord);
      const user = getMemoryUser(userId);
      if (user && !user.activeSessions.includes(sessionId)) {
        user.activeSessions = [...user.activeSessions, sessionId];
      }
      console.log(`[Session] \u2713 Created session in memory: ${sessionId}`);
      return sessionId;
    }
    const query = `
...
async function updateSession(sessionId, questionsAsked, voiceMinutesUsed = 0) {
  try {
    ...
    if (memorySession) {
      memorySession.questionsAsked = questionsAsked;
      memorySession.voiceMinutesUsed = voiceMinutesUsed;
      persistMemorySession(memorySession);
    }
    if (!isDBConnected()) {
      console.log(`[Session] \u2713 Updated in memory: ${questionsAsked} questions, ${voiceMinutesUsed}m voice`);
      return;
    }
    const query = `
      UPDATE sessions 
...
async function closeSession(sessionId, status) {
  try {
    const memorySession = memorySessions.get(sessionId);
    if (memorySession) {
      memorySession.status = status;
      memorySession.endTime = Date.now();
      persistMemorySession(memorySession);
      const user = getMemoryUser(memorySession.userId);
      if (user) {
        user.activeSessions = user.activeSessions.filter((id) => id !== sessionId);
        user.sessionHistory = [
          { sessionId: memorySession.sessionId, startTime: memorySession.startTime, endTime: memorySession.endTime, questionsAsked: memorySession.questionsAsked, voiceMinutesUsed: memorySession.voiceMinutesUsed, status: memorySession.status },
          ...user.sessionHistory
        ].slice(0, 50);
      }
    }
    if (!isDBConnected()) {
      console.log(`[Session] \u2713 Closed session in memory: status=${status}`);
      return;
    }
    const query = `
      UPDATE sessions 
...
async function getActiveSessions() {
  try {
    if (!isDBConnected()) {
      return Array.from(memorySessions.values()).filter((session) => session.status === "active").sort((a, b) => b.startTime - a.startTime).map((session) => toPublicSession(session, getMemoryUser(session.userId)?.email || ""));
    }
    const query = `
      SELECT
        s.session_id,
...
async function getUserSessionHistory(userId) {
  try {
    if (!isDBConnected()) {
      return Array.from(memorySessions.values()).filter((session) => session.userId === userId).sort((a, b) => b.startTime - a.startTime).slice(0, 50).map((session) => toPublicSession(session, getMemoryUser(session.userId)?.email || ""));
    }
    const query = `
      SELECT
        session_id,
...
async function recordChatUsage(userId, chatCount = 1) {
  ...
  try {
    ...
    persistMemoryUser(user);
    if (!isDBConnected()) {
      console.log(`[Usage] \u2713 Chat usage recorded in memory: ${user.chatMessagesUsed}/${PLAN_LIMITS[user.plan].chatMessagesPerMonth}`);
      return;
    }
...
async function recordVoiceUsage(userId, voiceMinutes) {
  ...
  try {
    ...
    persistMemoryUser(user);
    if (!isDBConnected()) {
      console.log(`[Usage] \u2713 Voice usage recorded in memory: ${user.voiceMinutesUsed}/${PLAN_LIMITS[user.plan].voiceMinutesPerMonth} minutes`);
      return;
    }
    const currentMonth = user.currentMonth;
    const query = `
...
async function upgradeUserPlan(userId, newPlan) {
  try {
    ...
    persistMemoryUser(user);
    if (!isDBConnected()) {
      console.log(`[DB] \u2713 User upgraded to plan in memory: ${newPlan}`);
      return user;
    }
    const query = `
      UPDATE users 
...
var memoryUsers, memorySessions;
  ...
  try {
    ...
    if (!userRecord) {
      try {
        ...
        console.warn("[Auth] \u26A0\uFE0F Using in-memory user fallback");
      }
    } else {
...
function getSectionHint(questionType) {
...
function getDepthHint(difficulty) {
  if (difficulty === "easy") {
...
function getPersonaAdjustments(persona) {
  return [
...
function buildChatSystemPrompt({ questionType, difficulty, resume, jd, persona }) {
...
function buildAnswerConfidencePrompt(question, answer) {
...
function buildAnswerVerificationPrompt(question, answer) {
...
function buildVoiceSystemPrompt({ resume, jd, persona }) {
...
function buildVoiceQuestionConfidencePrompt(transcript) {
...
function buildCacheQuestionsPrompt(jd) {
...
function buildCacheAnswerPrompt({ question, resume, jd }) {
...
Job Context: ${normalizeContextValue(jd).slice(0, 1e3)}`;
}
...
Job Context: ${jd.substring(0, 1e3)}` }
          ...
          const emb = await getEmbedding(q2);
          vectorCache.push({
          ...
          console.log(`[Cache] Pre-generated: ${q2.substring(0, 45)}... with ${variants.length} variations`);
        } catch (e) {
          console.log(`[Cache] Skipped individual generation for: ${q2}`);
        }
      }
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    const buildPath = import_path.default.join(__dirname, "../../build");
    app.use(import_express.default.static(buildPath));
    app.get("*", (req, res) => {
